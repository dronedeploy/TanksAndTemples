...
def transform_model(pcd_file, out_pcd_file, transform_file, invert_transform=False, is_pointcloud=False, verbose=True):
    if verbose:
...
if __name__ == "__main__":
    ...
    is_pointcloud = False
    if len(sys.argv) > 5:
        is_pointcloud = bool(sys.argv[5])
    transform_model(pcd_file, out_pcd_file, transform_file, invert_transform, is_pointcloud)
